...
public class EmployeesController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<EmployeeDto>>> GetEmployee(int id)
    {
        ...
        if (employee == null)
            return NotFound(new ApiResponse<EmployeeDto> { Success = false, Message = "Funcion�rio n�o encontrado" });
        ...
        IFormFile? photo)
    {
        ...
        if (await _context.Employees.AnyAsync(e => e.Email == employeeDto.Email && e.CompanyId == companyId))
            return BadRequest(new ApiResponse<EmployeeDto> { Success = false, Message = "E-mail j� existe nesta empresa" });
        var employee = new Employee
        {
            ...
            PasswordHash = BCrypt.Net.BCrypt.HashPassword("temp_password"), // Hash tempor�rio para PasswordHash
            EmailVerified = false,
            PhotoUrl = "/uploads/employees/default-avatar.png" // Define a foto padr�o por padr�o
        };
        // Substitui a foto padr�o por uma nova se enviada
    ...
    public async Task<ActionResult<ApiResponse<EmployeeDto>>> UpdateEmployee(int id, [FromForm] CreateEmployeeDto employeeDto, IFormFile? photo)
    {
        var existingEmployee = await _context.Employees
            .Include(e => e.Role)
            .FirstOrDefaultAsync(e => e.Id == id);
        if (existingEmployee == null) return NotFound(new ApiResponse<EmployeeDto> { Success = false, Message = "Funcion�rio n�o encontrado" });
        var companyId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
        if (existingEmployee.CompanyId != companyId) return Forbid();
        // Atualiza campos apenas se o formul�rio for completo
        existingEmployee.Name = employeeDto.Name;
        existingEmployee.Email = employeeDto.Email;
        existingEmployee.Phone = employeeDto.Phone;
        existingEmployee.RoleId = employeeDto.RoleId;
        // Mant�m a foto existente ou usa a padr�o s� se nunca houve foto
        if (string.IsNullOrEmpty(existingEmployee.PhotoUrl))
        {
            existingEmployee.PhotoUrl = "/uploads/employees/default-avatar.png";
        }
        // Substitui a foto (existente ou padr�o) por uma nova se enviada
        if (photo != null && photo.Length > 0)
        {
            ...
            // Deleta foto antiga se existir e n�o for a padr�o
    ...
    public async Task<IActionResult> DeleteEmployee(int id)
    {
        var employee = await _context.Employees
            .FirstOrDefaultAsync(e => e.Id == id);
        if (employee == null) return NotFound(new ApiResponse<Employee> { Success = false, Message = "Funcion�rio n�o encontrado" });
        var companyId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
        ...
        if (hasPendingAppointments)
            return BadRequest(new ApiResponse<Employee> { Success = false, Message = "Funcion�rio tem agendamentos pendentes" });
        // Deleta foto se existir e n�o for a padr�o
        ...
        await _context.SaveChangesAsync();
        return Ok(new ApiResponse<Employee> { Success = true, Message = "Funcion�rio deletado com sucesso" });
    }
    ...
    public async Task<ActionResult<ApiResponse<object>>> SendVerificationEmail(int id)
    {
        ...
        return Ok(new ApiResponse<object> { Success = true, Message = "Email de verifica��o reenviado!" });
    }

    // NOVO: GET an�nimo pra dados de ativa��o (valida token)
    [AllowAnonymous]
    ...
    public async Task<ActionResult<ApiResponse<ActivationDataDto>>> GetActivationData(int id, [FromQuery] string token)
    {
        ...
        if (employee == null)
            return NotFound(new ApiResponse<ActivationDataDto> { Success = false, Message = "Conta n�o encontrada ou j� ativada." });

        // Valida token simples (sem expira��o por agora)
        if (!ValidateTempToken(employee.Email ?? "", token))
            return Unauthorized(new ApiResponse<ActivationDataDto> { Success = false, Message = "Token inv�lido. Verifique o link do e-mail." });
    ...
    // NOVO: POST an�nimo pra ativar com senha
    [AllowAnonymous]
    ...
    public async Task<ActionResult<ApiResponse<EmployeeDto>>> ActivateEmployee(int id, [FromBody] ActivateEmployeeDto dto)
    {
        ...
        var employee = await _context.Employees.FirstOrDefaultAsync(e => e.Id == id && !e.EmailVerified);
        if (employee == null)
            return NotFound(new ApiResponse<EmployeeDto> { Success = false, Message = "Conta n�o encontrada ou j� ativada." });

        // Valida token (sem expira��o por agora)
        if (!ValidateTempToken(employee.Email ?? "", dto.Token))
            return Unauthorized(new ApiResponse<EmployeeDto> { Success = false, Message = "Token inv�lido. Solicite um novo e-mail." });
        ...
        return Ok(new ApiResponse<EmployeeDto> { Success = true, Data = activatedDto, Message = "Conta ativada com sucesso! Voc� pode fazer login." });
    }
    ...
    private async Task<(bool Success, string PhotoUrl, string Message)> UploadPhotoAsync(IFormFile photo, string? employeeId)
    {
        ...
        if (!allowedExtensions.Contains(extension))
            return (false, "", "Apenas imagens JPG, PNG ou GIF s�o permitidas");
        if (photo.Length > 5 * 1024 * 1024) // 5MB
            return (false, "", "Arquivo muito grande (m�x 5MB)");
        try
        {
            var uploadsFolder = Path.Combine(_env.WebRootPath, "uploads", "employees");
            if (!Directory.Exists(uploadsFolder))
            {
                Directory.CreateDirectory(uploadsFolder); // Cria a pasta se n�o existir
                Console.WriteLine($"Pasta criada: {uploadsFolder}"); // Debug
            }
            // Nome �nico: employeeId_timestamp_extension (se employeeId null, usa "new")
    ...
    // Helper: Constr�i URL completa (din�mica pra dev/prod)
    ...
    // NOVO: Helper pra validar token (com coment�rio pra expira��o)
    private bool ValidateTempToken(string email, string providedToken)
    {
        var expectedToken = GenerateTempToken(email);
        // Pra expira��o de 24h: armazene o timestamp no DB ao criar, e compare aqui (ex: if (DateTime.Now - createdAt > TimeSpan.FromHours(24)) return false;)
...
<body>
    <div class='container'>
        ...
        <p>Voc� foi convidado para a equipe no VEA - Veja, Explore e Agende, o sistema que automatiza agendamentos e centraliza a gest�o de atendimentos. Ative sua conta para acessar sua agenda e servi�os  � facilite o dia a dia da empresa com efici�ncia e visibilidade!</p>
        <a href='{inviteLink}' class='button'>Ativar e Criar Senha</a>
        <p class='link-fallback'>Se o bot�o n�o funcionar, copie e cole este link no navegador:<br><small>{inviteLink}</small></p>
        <p>Essa ativa��o expira em 24h. Qualquer d�vida, responda este email!</p>
        <div class='footer'>
