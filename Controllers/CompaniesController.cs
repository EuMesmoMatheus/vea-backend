...
public class CompaniesController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<CompanyDto>>> GetCompany(int id)
    {
        ...
        if (company == null)
            return NotFound(new ApiResponse<CompanyDto> { Success = false, Message = "Empresa n�o encontrada" });
    ...
    public async Task<ActionResult<ApiResponse<Company>>> RegisterCompany([FromForm] CompanyDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.Name) || string.IsNullOrWhiteSpace(dto.Email) ||
            ...
            return BadRequest(new ApiResponse<Company> { Success = false, Message = "Nome, email, telefone e senha s�o obrigat�rios." });

        if (await _context.Companies.AnyAsync(c => c.Email == dto.Email))
            return BadRequest(new ApiResponse<Company> { Success = false, Message = "E-mail j� existe." });
            ...
            !string.IsNullOrEmpty(dto.Cidade) && !string.IsNullOrEmpty(dto.Uf))
        {
            if (!Regex.IsMatch(dto.Cep, @"^\d{5}-\d{3}$"))
                return BadRequest(new ApiResponse<Company> { Success = false, Message = "CEP inv�lido. Use formato 00000-000" });
    ...
    public async Task<IActionResult> UpdateCompany(int id, [FromForm] CompanyDto dto)
    {
        ...
        if (company == null)
            return NotFound(new ApiResponse<Company> { Success = false, Message = "Empresa n�o encontrada" });
    ...
    public async Task<IActionResult> DeleteCompany(int id)
    {
        ...
        if (company.Employees.Any())
            return BadRequest(new ApiResponse<Company> { Success = false, Message = "N�o � poss�vel deletar: empresa possui funcion�rios" });

        var hasActiveAppointments = await _context.Appointments
            .AnyAsync(a => a.CompanyId == id && (a.Status == "Scheduled" || a.Status == "Pending"));

        if (hasActiveAppointments)
            return BadRequest(new ApiResponse<Company> { Success = false, Message = "N�o � poss�vel deletar: empresa possui agendamentos ativos" });
